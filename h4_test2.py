if __name__ == '__main__':
    ...
    pool, v_pool = s.uccsd_pool(spin_adapt = False)
    
    
    xiphos = Xiphos(H, ref, "h4_extended", pool, v_pool, sym_ops = {"H": H, "S_z": Sz, "S^2": S2, "N": Nop}, verbose = "DEBUG")
    params = list(np.load("h4_single/params.npy"))
    ansatz = list(np.load("h4_single/ops.npy"))
    xiphos.adapt(params, ansatz, ref, max_depth = 35, guesses = 125)
