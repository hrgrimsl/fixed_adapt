if __name__ == '__main__':
    ...
    xiphos = Xiphos(H, ref, "h4_wfn", pool, v_pool, sym_ops = {"H": H, "S_z": Sz, "S^2": S2, "N": Nop}, verbose = "DEBUG")
    params = np.array([])
    ansatz = []
    xiphos.adapt(params, ansatz, ref, max_depth = 1, guesses = 0)
    ansatz = list(np.load("h4_wfn/ops.npy"))
    op = xiphos.pool[ansatz[0]]
    wfn_grid(op, xiphos.pool, xiphos.ref, xiphos)
