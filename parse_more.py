...
plt.scatter(x[-1]*np.ones(len(random_Es[-1])), np.array(random_Es[-1]) - ci, label = "BFGS Using 2000 Random Initializations", alpha = .3, c = idx, cmap = "Spectral_r") 
...
#plt.axvline(x = 4, linewidth = 1, color = "red", label = "First cycles in directed graph of basis states.")
#plt.axvline(x = 10, linewidth = 1, color = "green", label = "First repeated operator.")
#plt.axvline(x = 18, linewidth = 1, color = "blue", label = "First doubly repeated operator.")
...
plt.title("ADAPT-VQE on H6, 3$\AA$ Bond Length, Spin-Adapted UCCSD Pool")
plt.legend()
plt.savefig("big.pdf")
plt.show()
exit()
...
plt.scatter(df["Ops"], df["E"]-df["Exact"], label = 'Solutions to 2000 Different BFGS Initializations in (-1,1)^N', alpha = .2, s = 1)
